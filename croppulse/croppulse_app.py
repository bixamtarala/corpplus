...
def calculate_historical_risk_scores(commodity_data):
    ...
    for i in range(7, len(commodity_data)):
        ...
        volatility = window['price'].std()
        price_change = (window['price'].iloc[-1] - window['price'].mean()) / window['price'].mean()
        supply_gap = 100 - window['supply'].iloc[-1]
        
        risk = (abs(volatility) * 0.4 + 
                abs(price_change) * 0.3 + 
                (supply_gap / 100) * 0.3 * 100)
...
def get_risk_components(commodity_data):
    ...
    if price_mean == 0:
        return {}
    price_change = ((commodity_data['price'].iloc[-1] - price_mean) / 
                    price_mean * 100)
    ...
    price_change_score = min(abs(price_change * 2), 100)
    supply_gap_score = 100 - supply
...
def predict_risk_trend(commodity_data):
    ...
    recent_volatility = commodity_data['price'].iloc[-7:].std()
    older_volatility = commodity_data['price'].iloc[-14:-7].std()
...
def get_price_trend_category(price_change_7d):
    ...
    if price_change_7d > 5:
        ...
        return "Moderate Downtrend", "#f39c12"
    else:
...
def generate_insights(commodity_data, risk_score, commodity):
    """Generate rule-based AI insights (Phase 4: Enhanced)"""
    insights = []
    ...
    if price_change_7d > 5 and price_change_14d > 8:
        ...
        insights.append({
            ...
            "description": f"Price down {price_change_7d:.1f}% in 7 days. Excess supply detected.",
            "action": "Hold off on selling; wait for recovery",
            "type": "caution",
            "confidence": 0.68
        })
    
    # Rule 2: Volatility analysis
    if volatility > 7:
        insights.append({
            "emoji": "⚠️",
            "title": "Extreme Volatility Warning",
            "description": f"Price volatility at {volatility:.1f}%. Highly unpredictable market.",
        ...
        insights.append({
            ...
            "description": f"Price swings at {volatility:.1f}%. Market uncertainty present.",
            "action": "Lock in prices or use hedging strategies",
            "type": "caution",
            "confidence": 0.75
        })
    
    # Rule 3: Price positioning
    if current_price > price_avg * 1.08:
        insights.append({
            "emoji": "📊",
            "title": "Above Average Pricing",
            "description": f"Price is +{((current_price - price_avg) / price_avg * 100):.1f}% above 30-day average.",
            "action": "Peak pricing window - strong sell opportunity",
            "type": "opportunity",
            "confidence": 0.78
        })
    elif current_price < price_avg * 0.92:
        insights.append({
            "emoji": "📊",
            "title": "Below Average Pricing",
            "description": f"Price is -{((price_avg - current_price) / price_avg * 100):.1f}% below 30-day average.",
            "action": "Poor sell conditions; wait for recovery to average",
            "type": "caution",
            "confidence": 0.74
        })
    
    # Rule 4: Supply shortage
    if supply < 35:
        insights.append({
            "emoji": "📦",
    ...
    # Rule 6: Composite patterns
    if supply < 50 and demand > 80 and price_change_7d > 3:
        insights.append({
            "emoji": "🎯",
            "title": "Perfect Storm: Shortage + Demand + Rising",
            "description": "Rare: Supply shortage + high demand + rising prices.",
            "action": "Extremely favorable - Sell immediately if possible",
            "type": "opportunity",
            "confidence": 0.92
        })
    
    if volatility > 6 and risk_score > 70:
        insights.append({
            "emoji": "⚠️",
            "title": "High Risk + Volatility",
            "description": "Combined high risk and volatility indicate uncertain market.",
            "action": "Defer selling; reduce exposure",
            "type": "caution",
            "confidence": 0.87
        })
...
with col3:
    ...
    <div class="metric-card">
        ...
        <div style='font-size: 32px; font-weight: 700; color: #2c3e50; margin: 8px 0;'>{volatility:.1f}%</div>
...
# ============================================================================
# RISK ASSESSMENT (PHASE 3: ADVANCED)
# ============================================================================
...
with col_risk2:
    ...
    st.markdown(f"""
    <div style='background: linear-gradient(135deg, #f5f7fa 0%, #ffffff 100%); padding: 20px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);'>
        <h4 style='margin-top: 0; color: #2c3e50;'>Risk Factors Breakdown</h4>
        
        <div style='margin: 16px 0;'>
            <div style='display: flex; justify-content: space-between; margin-bottom: 4px;'>
                <span style='color: #7f8c8d; font-size: 12px;'>📊 Volatility (40%)</span>
                <span style='color: #e74c3c; font-weight: 600; font-size: 12px;'>{components['volatility']:.1f}%</span>
            </div>
            <div class='progress-bar'>
                <div class='progress-fill' style='width: {min(components['volatility_score'], 100)}%; background-color: #e74c3c;'></div>
            </div>
        </div>
        
        <div style='margin: 16px 0;'>
            <div style='display: flex; justify-content: space-between; margin-bottom: 4px;'>
                <span style='color: #7f8c8d; font-size: 12px;'>💹 Price Change (30%)</span>
                <span style='color: #f39c12; font-weight: 600; font-size: 12px;'>{abs(components['price_change']):.1f}%</span>
            </div>
            <div class='progress-bar'>
                <div class='progress-fill' style='width: {min(components['price_change_score'], 100)}%; background-color: #f39c12;'></div>
            </div>
        </div>
        
        <div style='margin: 16px 0;'>
            <div style='display: flex; justify-content: space-between; margin-bottom: 4px;'>
                <span style='color: #7f8c8d; font-size: 12px;'>📦 Supply Gap (30%)</span>
                <span style='color: #3498db; font-weight: 600; font-size: 12px;'>{components['supply_gap']:.0f}%</span>
            </div>
            <div class='progress-bar'>
                <div class='progress-fill' style='width: {components['supply_gap_score']}%; background-color: #3498db;'></div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
